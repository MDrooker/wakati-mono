...
export interface S3Service {
    generateDownloadUrl(key: string): Promise<{ downloadUrl: string }>;
    uploadFile(params: S3UploadParams): Promise<void>;
}
...
export class VideoConversionService {
    ...
    async convertVideoForRekognition(
        ...
        if (!this.s3Service) {
            throw new Error('S3Service is required for video conversion');
        }
        ...
        try {
            this.logger.log(`Starting video conversion for ${bucketName}/${objectKey}`);

            // Step 1: Download the video from S3
            const downloadUrl = await this.s3Service.generateDownloadUrl(objectKey);
            tempInputFile = await this.downloadVideoToTemp(downloadUrl.downloadUrl, objectKey);
            ...
            if (this.isRekognitionCompatible(videoInfo)) {
                this.logger.log(`Video ${objectKey} is already compatible with Rekognition`);
                return {
                    success: true,
            ...
            this.logger.log(`Uploading converted video to ${bucketName}/${convertedKey}`);
            await this.uploadConvertedVideo(tempOutputFile, bucketName, convertedKey, userurn);

            const conversionTime = Date.now() - startTime;
            this.logger.log(`Video conversion completed in ${conversionTime}ms: ${convertedKey}`);
     ...
     */
    isRekognitionCompatible(videoInfo: VideoInfo): boolean {
        ...
        const isAudioCodecOk = !videoInfo.hasAudio || videoInfo.audioCodec === 'aac';
     ...
     */
    private async convertVideo(inputPath: string, videoInfo: VideoInfo): Promise<string> {
     ...
     */
    private async downloadVideoToTemp(url: string, originalKey: string): Promise<string> {
    ...
    private async uploadConvertedVideo(
        ...
        if (!this.s3Service) {
            throw new Error('S3Service is required for video upload');
        }
     ...
     */
    async checkFFmpegAvailability(): Promise<{ ffmpeg: boolean; ffprobe: boolean }> {
     ...
     */
    getFFmpegInstallationInstructions(): string[] {
        return [
            'FFmpeg is required for video conversion. Install it using:',
            '',
            ...
            '',
            'Docker: Use a base image with FFmpeg pre-installed or add it to your Dockerfile:',
            'RUN apt-get update && apt-get install -y ffmpeg',
            '',
            'For AWS Lambda: Use a layer with FFmpeg or include it in your deployment package'
        ];
    }
}
